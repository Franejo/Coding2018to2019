...
namespace Sudoku
{
    public class SudokuBoard
    {
        ...
        public SudokuBoard(string fileName)
        {
            ...
            int index = 0;
            string NewF = file[rand.Next(0, file.Count)];
            for (int i = 0; i < 9; i++)
            {
                for (int j = 0; j < 9; j++)
                {
                    Board[i, j] = int.Parse(NewF[index].ToString());
                    index++;
                }
            }
        }
